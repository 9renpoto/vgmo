...
import { scrapeConcertPage, mergeConcerts } from "../src/main.ts";
...
test("scrapeConcertPage should parse HTML and extract concert details", async (t) => {
  ...
  t.mock.method(global, "fetch", (url: string) => {
    ...
    // The main page fetch
    return Promise.resolve(new Response(sjisBuffer, {
      status: 200,
  ...
  assert.strictEqual(concert1.sourceUrl, "https://www.2083.jp/concert/concert-1.html");
  ...
  assert.strictEqual(concert2.sourceUrl, "https://www.2083.jp/concert/concert-2.html");
  ...
  assert.strictEqual(concert3.imageUrl, undefined, "Image URL should be undefined for concert 3");
});
